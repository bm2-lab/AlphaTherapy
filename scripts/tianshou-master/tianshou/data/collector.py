...
class Collector(object):
    ...
    def __init__(self,
        ...
        self.prev_act = -1  # modify: prev_act is used to record the drug index of the second drug (since when the second drug is chosen, only one drug remains in the action space).
    ...
    def collect(self,
        ...
        diff_sum, eff_sum =0., 0 # modify: record diff_score and eff_score
        while True:
            ...
            # modify: When there is only one drug left, it can only be selected.
            ...
            # modify: When the first drug is chosen
            if action_num != 1:
                self.prev_act = self.data.act
            ...
            # modify: When the second drug is chosen
            if action_num == 1:
                self.data.act = self.prev_act
            ...
            if self._multi_env:  # cache_buffer branch
                ...
                cur_step += 1
                if self.data.done[0]: # 当一个episode结束的时候
                    cur_episode += 1
                    reward_sum += self.reward[0]
                    length_sum += self.length[0]
                    # modify
                    diff_sum += self.data.info[0]["diff"]
                    eff_sum += self.data.info[0]["eff"]
        ...
        diff_sum /= n_episode  # modify
        eff_sum /= n_episode  # modify
    ...
    def sample(self, batch_size: int) -> Batch:
        ...
        batch_data = self.process_fn(batch_data, self.buffer, indice)   # dqn policy中 process_fn
        return batch_data

    # collect_best和reset_best 是为了记录trajectory
    def collect_one_best(self,
                n_step: int = 0,
                n_episode: Union[int, List[int]] = 0,
                random: bool = False,
                render: Optional[float] = None,
        ...
        while True:
            ...
            if self._multi_env:  # cache_buffer branch
                ...
                obs_next = self.data.obs_next
                if sum(self.data.done):
                    # modify: Change reset to reset_best.
                    obs_next = self.env.reset_best(np.where(self.data.done)[0])
                    if self.preprocess_fn:
                        obs_next = self.preprocess_fn(obs=obs_next).get(
                            'obs', obs_next)
                self.data.obs_next = obs_next
                ...
                if self.data.done[0]:
                    ...
                    self.data.state = Batch()
                    # modify: Change reset to reset_best.
                    obs_next = self._make_batch(self.env.reset_best())
                    if self.preprocess_fn:
                        obs_next = self.preprocess_fn(obs=obs_next).get(
                            'obs', obs_next)
                    self.data.obs_next = obs_next
    ...
    def reset_env_iter(self) -> None:
        """Reset all of the environment(s)' states and reset all of the cache
        buffers (if need).
        """
        obs = self.env.reset(reset_cycle=True)
        if not self._multi_env:
            obs = self._make_batch(obs)
        if self.preprocess_fn:
            obs = self.preprocess_fn(obs=obs).get('obs', obs)
    ...
    def collect_iter(self,
        ...
        while True:
            ...
            if self._multi_env:  # cache_buffer branch
                ...
                if sum(self.data.done):
                    # modify: reset_iter
                    obs_next = self.env.reset(np.where(self.data.done)[0], reset_cycle=True)
                    if self.preprocess_fn:
                        obs_next = self.preprocess_fn(obs=obs_next).get(
                            'obs', obs_next)
                self.data.obs_next = obs_next
                ...
                if self.data.done[0]:
                    ...
                    self.data.state = Batch()
                    # modify: reset_iter
                    obs_next = self._make_batch(self.env.reset(reset_cycle=True))
                    if self.preprocess_fn:
                        obs_next = self.preprocess_fn(obs=obs_next).get(
                            'obs', obs_next)
                    self.data.obs_next = obs_next
