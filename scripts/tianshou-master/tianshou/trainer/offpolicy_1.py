...
def offpolicy_trainer(
    ...
    plot_train_data = pd.DataFrame(columns=['len','loss','reward'])
    plot_test_data = pd.DataFrame(columns=['len','reward'])
    stat = {}
    ...
    for epoch in range(1, 1 + max_epoch):
        ...
        with tqdm.tqdm(total=step_per_epoch, desc=f'Epoch #{epoch}',
            ...
            while t.n < t.total:
                ...
                for i in range(update_per_step * min(
                    ...
                    plot_train_data = plot_train_data.append(pd.DataFrame({'len':[result['len']],'loss':[stat['loss'].get()],'reward':[result['rew']]}),ignore_index=True)
                    t.update(1)
                    t.set_postfix(**data)
            if t.n <= t.total:
                t.update()
        ...
        # modify: plot_test_data
        plot_test_data = plot_test_data.append(pd.DataFrame({'len':[result['len']],'reward':[result['rew']]}),ignore_index=True)
